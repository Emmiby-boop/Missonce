<template>
  <div class="space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
      <div>
        <h1 class="text-2xl font-bold text-[var(--text-main)]">媒体解析管理</h1>
        <p class="text-[var(--text-sub)] mt-1">解析工具与平台状态一览</p>
      </div>
    </div>

    <!-- Tab Switcher (same style as CategoriesTagsPage) -->
    ...
    <div v-if="activeTab === 'parse'" class="space-y-6">
      ...
      <div class="glass-panel">
        <div class="flex flex-col gap-4">
          <div class="flex flex-col md:flex-row gap-3">
            <div class="flex-1 relative">
              <input
                v-model="inputUrl"
                type="text"
                class="form-input w-full pl-11 pr-4"
                placeholder="粘贴视频/图文链接，支持抖音、小红书、快手、B站等..."
                @keydown.enter="handleParse"
                :disabled="parsing"
              />
              <svg class="absolute left-3.5 top-1/2 -translate-y-1/2 text-[var(--text-sub)]" xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            ...
            <button
              ...
              class="btn-soft flex items-center gap-2 whitespace-nowrap"
            >
              <svg v-if="parsing" class="animate-spin" xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
          ...
          <div class="flex flex-wrap gap-2">
            <span class="text-xs text-[var(--text-sub)]">快捷测试：</span>
            <button
              v-for="demo in demoLinks"
              :key="demo.label"
              @click="inputUrl = demo.url; handleParse()"
              class="text-xs px-2.5 py-1 rounded-full bg-[var(--bg-body)] border border-[var(--border-color)] text-[var(--text-sub)] hover:text-[var(--primary)] hover:border-[var(--primary)] transition-colors"
            >
              {{ demo.label }}
            </button>
          </div>
      ...
      <div v-if="parseError" class="glass-panel border-red-500/30 bg-red-500/5">
        <div class="flex items-center gap-3">
          ...
          <div>
            <p class="text-sm font-medium text-red-500">解析失败</p>
            <p class="text-xs text-[var(--text-sub)] mt-0.5">{{ parseError }}</p>
          </div>
        </div>
      </div>
      ...
      <div v-if="result" class="space-y-6">
        ...
        <div class="glass-panel">
          <div class="flex items-start justify-between gap-4">
            <div class="flex-1 min-w-0">
              <div class="flex items-center gap-3 mb-3">
                <span :class="getPlatformBadgeClass(result.platform)">
                  {{ result.platform }}
                </span>
                <span v-if="parseDuration" class="text-xs text-[var(--text-sub)]">
                  耗时 {{ parseDuration }}ms
                </span>
              </div>
              <h2 class="text-lg font-semibold text-[var(--text-main)] leading-relaxed">
                {{ result.title || '无标题' }}
              </h2>
            </div>
          </div>
          ...
          <!-- Author Info -->
          <div v-if="result.author && (result.author.name || result.author.nickname)" class="mt-4 flex items-center gap-3 p-3 rounded-lg bg-[var(--bg-body)]">
            <div class="w-10 h-10 rounded-full bg-[var(--primary)]/10 flex items-center justify-center text-[var(--primary)] overflow-hidden flex-shrink-0">
              <img v-if="result.author.avatar" :src="result.author.avatar" class="w-full h-full object-cover" alt="" />
              <svg v-else xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                <path d="M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/>
              </svg>
            </div>
            <div>
              <p class="text-sm font-medium text-[var(--text-main)]">{{ result.author.name || result.author.nickname }}</p>
              <p v-if="result.author.uid" class="text-xs text-[var(--text-sub)]">ID: {{ result.author.uid }}</p>
            </div>
          </div>
        </div>

        <!-- Video Section -->
        <div v-if="result.video_url" class="glass-panel">
          <div class="flex items-center justify-between mb-4">
            <div class="flex items-center gap-2">
              ...
              <h3 class="font-semibold text-[var(--text-main)]">视频</h3>
            </div>
          </div>
          <div class="rounded-lg overflow-hidden bg-black/5 border border-[var(--border-color)]">
            <video
              :src="getProxyUrl(result.video_url)"
              controls
              class="w-full max-h-[480px]"
              :poster="result.cover_url ? getProxyUrl(result.cover_url) : undefined"
            ></video>
          </div>
          <div class="mt-4 flex flex-wrap gap-3">
            <button @click="handleDownload(result.video_url, `video_${result.video_id || 'download'}.mp4`)" class="btn-soft flex items-center gap-2">
              <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" x2="12" y1="15" y2="3"/>
              </svg>
              下载视频
            </button>
            <button @click="copyToClipboard(result.video_url)" class="btn flex items-center gap-2">
              <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
        ...
        <!-- Audio Section -->
        <div v-if="result.audio_url" class="glass-panel">
          <div class="flex items-center justify-between mb-4">
            <div class="flex items-center gap-2">
              ...
              <h3 class="font-semibold text-[var(--text-main)]">音频</h3>
            </div>
          </div>
          <audio :src="getProxyUrl(result.audio_url)" controls class="w-full"></audio>
          <div class="mt-3 flex flex-wrap gap-3">
            <button @click="handleDownload(result.audio_url, `audio_${result.video_id || 'download'}.mp3`)" class="btn-soft flex items-center gap-2">
              <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" x2="12" y1="15" y2="3"/>
              </svg>
              下载音频
            </button>
            <button @click="copyToClipboard(result.audio_url)" class="btn flex items-center gap-2">
              <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                <rect width="14" height="14" x="8" y="8" rx="2"/><path d="M4 16c-1.1 0-2-.9-2-2V4c0-1.1.9-2 2-2h10c1.1 0 2 .9 2 2"/>
              </svg>
              复制链接
        ...
        <!-- Image Grid -->
        <div v-if="result.image_list && result.image_list.length > 0" class="glass-panel">
          <div class="flex items-center justify-between mb-4">
            <div class="flex items-center gap-2">
              ...
              <h3 class="font-semibold text-[var(--text-main)]">图片 ({{ result.image_list.length }})</h3>
            </div>
            <button @click="downloadAllImages" class="btn-soft text-xs flex items-center gap-1.5">
              <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" x2="12" y1="15" y2="3"/>
              </svg>
              全部下载
          ...
          <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
            <div
              v-for="(img, index) in result.image_list"
              :key="index"
              class="group relative rounded-lg overflow-hidden bg-[var(--bg-body)] border border-[var(--border-color)] aspect-square cursor-pointer hover:shadow-md transition-all"
            >
              <img
                :src="getProxyUrl(typeof img === 'object' ? img.url : img)"
                class="w-full h-full object-cover"
                loading="lazy"
                @click="previewImage(typeof img === 'object' ? img.url : img)"
              />
              <div class="absolute inset-0 bg-black/0 group-hover:bg-black/30 transition-all flex items-center justify-center opacity-0 group-hover:opacity-100">
                <div class="flex gap-2">
                  <button @click.stop="handleDownload(typeof img === 'object' ? img.url : img, `image_${index + 1}.jpg`)" class="p-2 rounded-lg bg-white/90 text-[var(--text-main)] hover:bg-white transition-colors">
                    <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                      <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" x2="12" y1="15" y2="3"/>
                    </svg>
                  </button>
              ...
              <div v-if="typeof img === 'object' && img.live_photo_url" class="absolute top-2 left-2 px-1.5 py-0.5 rounded bg-black/60 text-white text-xs flex items-center gap-1">
        ...
        <div class="glass-panel">
          <button @click="showRawData = !showRawData" class="flex items-center gap-2 w-full text-left">
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="text-[var(--text-sub)] transition-transform" :class="{ 'rotate-90': showRawData }">
              <path d="m9 18 6-6-6-6"/>
            </svg>
            <span class="text-sm font-medium text-[var(--text-sub)]">原始数据 (JSON)</span>
          </button>
          <div v-if="showRawData" class="mt-4">
            <pre class="p-4 rounded-lg bg-[var(--bg-body)] text-xs text-[var(--text-sub)] overflow-x-auto font-mono leading-relaxed">{{ JSON.stringify(result, null, 2) }}</pre>
          </div>
        </div>
      </div>

      <!-- Recent Parse History -->
      <div v-if="!result && !parsing && !parseError" class="glass-panel">
        <div class="flex items-center justify-between mb-4">
          <h3 class="font-semibold text-[var(--text-main)]">最近解析记录</h3>
          <button v-if="parseHistory.length > 0" @click="clearHistory" class="text-xs text-[var(--text-sub)] hover:text-red-500 transition-colors">清空</button>
        </div>
        <div v-if="parseHistory.length === 0" class="text-center py-8">
          <div class="w-12 h-12 rounded-full bg-[var(--bg-body)] mx-auto flex items-center justify-center mb-3">
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="text-[var(--text-sub)]">
              <circle cx="11" cy="11" r="8"/><path d="m21 21-4.3-4.3"/>
            </svg>
          </div>
          <p class="text-sm text-[var(--text-sub)]">暂无解析记录</p>
          <p class="text-xs text-[var(--text-sub)] mt-1">粘贴链接后点击解析即可开始</p>
        </div>
        <div v-else class="space-y-2">
          <div
            v-for="(item, i) in parseHistory.slice(0, 5)"
            :key="i"
            class="flex items-center gap-3 p-3 rounded-lg bg-[var(--bg-body)] hover:bg-[var(--bg-body)]/80 cursor-pointer transition-colors"
            @click="inputUrl = item.input; handleParse()"
          >
            <span :class="getPlatformBadgeClass(item.platform)">
              {{ item.platform }}
            </span>
            <span class="flex-1 text-sm text-[var(--text-main)] truncate">{{ item.title || item.input }}</span>
            <span class="text-xs text-[var(--text-sub)] whitespace-nowrap">{{ item.time }}</span>
          </div>
        </div>
      </div>
    </div>
    ...
    <!-- ==================== Tab 2: 平台状态 ==================== -->
    <div v-if="activeTab === 'platforms'" class="space-y-6">
      <!-- Stats Cards -->
    ...
    <!-- Image Preview Modal -->
    <div v-if="previewingImage" class="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4" @click="previewingImage = null">
      <img :src="getProxyUrl(previewingImage)" class="max-w-full max-h-[90vh] object-contain rounded-lg" @click.stop />
      <button @click="previewingImage = null" class="absolute top-4 right-4 p-2 rounded-full bg-white/10 hover:bg-white/20 text-white transition-colors">
        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
...
// API base URL for Media Toolkit backend
const API_BASE = 'https://api.missonce.cc'

// Tab state
const activeTab = ref<'parse' | 'platforms'>('parse')
const tabs = [
  { label: '解析工具', value: 'parse' },
  { label: '平台状态', value: 'platforms' }
]
...
const handleParse = async () => {
  ...
  try {
    const response = await fetch(`${API_BASE}/api/parse`, {
      ...
      body: JSON.stringify({ text: inputUrl.value.trim() }),
    })

    const data = await response.json()
    parseDuration.value = Date.now() - startTime

    if (data.succ && data.data) {
      ...
      saveHistory()
      ElMessage.success(`解析成功 · ${data.data.platform}`)
    } else {
      parseError.value = data.retdesc || '解析失败，请检查链接是否正确'
    }
  } catch (e: any) {
    parseError.value = e.message || '网络错误，请确保后端服务正在运行'
  } finally {
    parsing.value = false
  }
}
...
const handleDownload = (url: string, filename: string) => {
  if (!url) return
  const proxyUrl = `${API_BASE}/api/proxyDownload?url=${encodeURIComponent(url)}&filename=${encodeURIComponent(filename)}`
...
const downloadAllImages = () => {
  if (!result.value?.image_list) return
  result.value.image_list.forEach((img: any, i: number) => {
    const url = typeof img === 'object' ? img.url : img
    setTimeout(() => handleDownload(url, `image_${i + 1}.jpg`), i * 300)
  })
}
...
const clearHistory = () => {
  parseHistory.value = []
  localStorage.removeItem('media_parse_history')
  ElMessage.success('已清空')
}

const saveHistory = () => {
  try {
    localStorage.setItem('media_parse_history', JSON.stringify(parseHistory.value))
  } catch {}
}

const loadHistory = () => {
  try {
    const saved = localStorage.getItem('media_parse_history')
    if (saved) parseHistory.value = JSON.parse(saved)
  } catch {}
}
...
const getPlatformBadgeClass = (platform: string) => {
  ...
  const map: Record<string, string> = {
    '抖音': `${base} bg-blue-500/10 text-blue-500`,
    '小红书': `${base} bg-red-500/10 text-red-500`,
    '快手': `${base} bg-orange-500/10 text-orange-500`,
    '哔哩哔哩': `${base} bg-cyan-500/10 text-cyan-500`,
  }
  return map[platform] || `${base} bg-purple-500/10 text-purple-500`
}
...
interface Platform {
  name: string
  iconType: 'local' | 'svg'
  iconSrc: string  // local: path like /icons/xxx.ico; svg: SVG path data
...
// Inline SVG paths from Simple Icons (open source, zero network dependency)
...
// Platforms: domestic platforms use local favicons (in /public/icons/),
// international platforms + key domestic platforms use inline SVG from Simple Icons,
// remaining platforms without real icons use local files with onIconError fallback
const platforms: Platform[] = [
  // --- Implemented: platforms with Simple Icons SVG ---
  ...
  // --- Implemented: domestic platforms (local favicons) ---
  ...
  // --- Pending: international platforms (inline SVG) ---
...
// Fallback for failed local icon loads — show first char with brand color
