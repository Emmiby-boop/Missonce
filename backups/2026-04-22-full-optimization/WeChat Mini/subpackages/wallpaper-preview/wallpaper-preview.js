...
Page({
  ...
  showAdLoading(kind = 'rewarded') {
    ...
    try { getApp().logEvent('ad_loading_show', { kind, page: 'wallpaper-preview' }) } catch (e) {}
    ...
    this._adLoadingTimer = setTimeout(() => {
      if (this.data.showAdLoading) {
        ...
        try { getApp().logEvent('ad_loading_timeout', { kind, page: 'wallpaper-preview' }) } catch (e) {}
      }
      this._adLoadingTimer = null
    }, 10000)
  },

  hideAdLoading(kind = 'rewarded', reason = '') {
    if (this._adLoadingTimer) { clearTimeout(this._adLoadingTimer); this._adLoadingTimer = null }
    if (this.data.showAdLoading) {
      this.setData({ showAdLoading: false })
    }
    try { getApp().logEvent('ad_loading_hide', { kind, reason, page: 'wallpaper-preview' }) } catch (e) {}
  },
