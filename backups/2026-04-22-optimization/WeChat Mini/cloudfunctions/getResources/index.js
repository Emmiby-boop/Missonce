...
exports.main = async (event) => {
  ...
  try {
    ...
    if (ids && ids.length > 0) {
       const res = await db.collection('resources')
        ...
        .field({
          ...
          hotScore: true, dailyHotScore: true, downloads: true, favorites: true, createdAt: true
        })
        .limit(100)
        .get()
    ...
    if (searchKeyword) {
      // 定义类型关键词
      const avatarKeywords = ['头像', '头象']
      const wallpaperKeywords = ['壁纸', '背景', '墙纸']
      
      let hasAvatarKeyword = false
      let hasWallpaperKeyword = false
      
      // 检查是否包含类型关键词
      avatarKeywords.forEach(k => {
        if (searchKeyword.includes(k)) hasAvatarKeyword = true
      })
      wallpaperKeywords.forEach(k => {
        if (searchKeyword.includes(k)) hasWallpaperKeyword = true
      })
      
      // 如果同时包含两种类型关键词，不做处理，保留原始搜索
      if (hasAvatarKeyword && hasWallpaperKeyword) {
        // 不处理
      } 
      // 如果包含头像关键词
      else if (hasAvatarKeyword) {
        searchType = 'avatar'
        // 移除类型关键词
        avatarKeywords.forEach(k => {
          searchKeyword = searchKeyword.split(k).join('')
        })
      } 
      // 如果包含壁纸关键词
      else if (hasWallpaperKeyword) {
      ...
      // 清理关键词
      const cleanKeyword = searchKeyword.replace(/\s+/g, ' ').trim()
      
      if (cleanKeyword) {
        const keywordReg = db.RegExp({ regexp: cleanKeyword, options: 'i' })
        conditions.push(_.or([
          // 1. 精确匹配 categories 数组（和后台一样）
          { categories: _.in([cleanKeyword]) },
          // 2. 精确匹配 tags 数组（和后台一样）
          { tags: _.in([cleanKeyword]) },
          // 3. 正则匹配 title
          { title: keywordReg },
          // 4. 正则匹配 categories 数组（备选）
          { categories: keywordReg },
          // 5. 正则匹配 tags 数组（备选）
          { tags: keywordReg },
          // 6. 正则匹配 category 字符串（备选）
          { category: keywordReg }
        ]))
      }
    }
    ...
    // 🔥 优化：移除云函数端 URL 转换，返回原始 cloud:// URL
    // 原因：getTempFileURL 阻塞严重，导致响应慢
    // 改为客户端按需转换，利用微信小程序原生缓存机制
