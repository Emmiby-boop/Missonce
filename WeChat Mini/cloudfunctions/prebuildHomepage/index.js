...
const CloudCache = require('../shared/cache')
const cache = new CloudCache({ defaultTTL: 45 * 60 * 1000 }) // 预渲染缓存 45 分钟
...
async function generateSectionItems(db, sectionConfig, categoryMap) {
  ...
  if (sourceType === 'manual' && dataSource.manualItems && dataSource.manualItems.length > 0) {
    ...
    if (manualIds.length > 0) {
      ...
      items = manualIds.map(id => resourceMap[id]).filter(item => item).map(slimItem)
    }
  }
    ...
    if (resourceType === 'all' && !autoSplit) {
      ...
      const [wallpaperRes, avatarRes] = await Promise.all([
        db.collection('resources').where({ ...queryConditions, type: 'wallpaper' })
          .orderBy(sortFieldName, 'desc').field({ _id: true, title: true, type: true, coverUrl: true }).limit(totalLimit).get(),
      ...
      const result = await query.field({ _id: true, title: true, type: true, coverUrl: true }).limit(totalLimit).get()
  ...
  try {
    ...
    const [bannersResult, sectionsResult, categoriesResult] = await Promise.all([
      db.collection('banners').orderBy('sort', 'asc').where({ status: 'active' })
        .field({ _id: true, title: true, image: true, link: true, type: true, target: true }).get(),
    ...
    // 6. 上传 JSON 到云存储（供前端 wx.request 直连拉取，跳过 callFunction 链路）
    let cloudStorageUrl = null
    try {
      const jsonContent = JSON.stringify(result)
      const tmpPath = `/tmp/home_prebuilt_v1.json`
      fs.writeFileSync(tmpPath, jsonContent, 'utf8')
      const uploadRes = await cloud.uploadFile({
        cloudPath: 'miniprogram/home/home_prebuilt_v1.json',
        fileContent: fs.createReadStream(tmpPath)
      })
      cloudStorageUrl = uploadRes.fileID
      console.log(`[prebuildHomepage] 云存储上传成功: ${cloudStorageUrl}`)
    } catch (uploadErr) {
      console.warn('[prebuildHomepage] ⚠️ 云存储上传失败，不影响主流程:', uploadErr.message || uploadErr)
    }

    // 7. 写入预渲染缓存（云数据库，作为兜底）
    const prebuiltCacheKey = 'home_prebuilt_v1'
    await cache.set(prebuiltCacheKey, result, 45 * 60 * 1000)

    const cost = Date.now() - startTime
    console.log(`[prebuildHomepage] ✅ 预渲染完成，耗时: ${cost}ms`)
    console.log(`  - banners: ${banners.length}`)
    console.log(`  - sections: ${sectionsResult.data.length}`)
    console.log(`  - cloudStorageUrl: ${cloudStorageUrl || 'N/A'}`)
